...
namespace storage {
Buffermanager::Buffermanager(rdma::CM<rdma::InitMessage>& cm, NodeID nodeId, s32 ssd_fd, PageIdManager& pageIdManager)
...
void Buffermanager::readPageSync(PID pid, uint8_t* destination) {
   ...
   do {
      ...
      const int bytes_read = pread(ssd_fd, destination, bytes_left, ssdSlot * PAGE_SIZE + (PAGE_SIZE - bytes_left));
      assert(bytes_left > 0);
